...
if (isset($_POST['action'])) {
  switch($_POST['action']) {
    ...
    case 'toggle':
      ...
      $id = $_POST['id'];
      if(is_numeric($id)) {
        $updateQuery = 'UPDATE todo SET done = NOT done WHERE id ='.$id; // Modifier le statue de la tache
    ...
    case 'delete':
      ...
      $id = $_POST['id'];
      if(is_numeric($id)) {
        $deleteQuery = 'DELETE FROM todo WHERE id =' .$id; // Supprimer la tache
 ...
 */
$selectQuery = 'SELECT * FROM todo ORDER BY created_at DESC'; // Selectionner toutes les taches
